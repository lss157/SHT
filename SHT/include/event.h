 ...
 */
typedef struct{
	sht_res_t  res; 			///<event也是一种资源
  	unsigned char type; 		///<类型，SHT_EVENT_SEM（信号量）或SHT_EVENT_MUTEX（互斥量）
	...
	sht_list_t wait_queue; 	///<等待使用这个event的线程队列
	char*		  name; 		///<名字
	void*		  data; 		///<当event是mutex或Semaphore时，指向占用线程，当event是消息队列时，存放传递的消息
}sht_evt_t;

void sht_evt_sys_init(void);
void sht_evt_pool_init(void);
sht_evt_t *sht_alloc_evt(void);
