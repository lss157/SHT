...
#define sht_malloc(size) buddy_malloc(size)
#define sht_free(ptr) buddy_free(ptr)
#define sht_malloc_adjust_size(size) buddy_malloc_size(size)
#define sht_mem_init(start,end) buddy_init(start,end)
#define sht_mem_scan() buddy_scan()
...
#define SHT_POOL_INDEX_BIT 0
#define SHT_POOL_ID_BIT 0

#define SHT_RES_TYPE_BIT 10
#define SHT_RES_INDEX_BIT 14
#define SHT_RES_INDEX_INIT_BIT 16
#define SHT_RES_ID_BIT 24

#define SHT_POOL_INDEX_MASK 0x000003FF
#define SHT_POOL_ID_MASK    0x00003FFF

#define SHT_RES_TYPE_MASK   0x00003c00
#define SHT_RES_INDEX_MASK  0x00FFC000
#define SHT_RES_ID_MASK     0x00FFFFFF
...
typedef enum{
   ...
   SHT_RES_NO_MEM,
   SHT_RES_MAX_POOL
}shtResourceReturnValEnum;

#define SHT_RES_TYPE(id) ((id&SHT_RES_TYPE_MASK)>>SHT_RES_TYPE_BIT) ///<根据资源ID获取某一资源数据块
...
typedef struct {
  ...
  sht_list_t *free_pools;
  sht_list_t *pools,list[2];
  unsigned char *name;
}sht_pool_ctrl_t;
...
typedef struct {
   ...
   sht_pool_ctrl_t *ctrl;
   sht_list_t ctrl_list;
   sht_list_t free_list;
}sht_pool_t;
 ...
 *
 * @param res_id 资源ID
 * @return sht_pool_t* 获取到的资源池指针
 */
sht_pool_t *sht_get_pool_by_id(int id);

/**
 * @brief 获取空闲资源池
 *
 * @return sht_pool_t* 获取到的空闲资源池指针
 */
sht_pool_t *sht_get_free_pool(void);
 ...
 * 		and then initialize sht_free_res_pool as the first pool.
 *
 */
void sht_res_sys_init(void);
