...
/**
  ...
  */
void PingPong_Process(void *args);
...
//  APP_LOG(TS_OFF, VLEVEL_M, "\n\rPING PONG\n\r");
  /* Get SubGHY_Phy APP version 打印应用版本信息  */
//  APP_LOG(TS_OFF, VLEVEL_M, "APPLICATION_VERSION: V%X.%X.%X\r\n",
//          (uint8_t)(APP_VERSION_MAIN),
//          (uint8_t)(APP_VERSION_SUB1),
//          (uint8_t)(APP_VERSION_SUB2));

  /* Get MW SubGhz_Phy info 打印SubGHz_Phy模块版本信息 */
//  APP_LOG(TS_OFF, VLEVEL_M, "MW_RADIO_VERSION:    V%X.%X.%X\r\n",
//          (uint8_t)(SUBGHZ_PHY_VERSION_MAIN),
//          (uint8_t)(SUBGHZ_PHY_VERSION_SUB1),
//          (uint8_t)(SUBGHZ_PHY_VERSION_SUB2));

  /* Led Timers 初始化LED定时器 创建一个定时器对象timerLed，模式为单次触发，指定回调函数为OnledEvent 最后启动定时器 */
  UTIL_TIMER_Create(&timerLed, LED_PERIOD_MS, UTIL_TIMER_ONESHOT, OnledEvent, NULL);
  UTIL_TIMER_Start(&timerLed);
  /* USER CODE END SubghzApp_Init_1 */

  /* Radio initialization Radio模块初始化，配置Radio事件处理函数，包括传输完成、接收完成、传输超时、接收超时和接收错误 */
  ...
  /*calculate random delay for synchronization计算一个随机延迟时间，用于同步操作，取值范围（0~1023）  */
  random_delay = (Radio.Random()) >> 22; /*10bits random e.g. from 0 to 1023 ms*/

  /* Radio Set frequency Radio设置频率和配置*/
  Radio.SetChannel(RF_FREQUENCY);

  /* Radio configuration */
  /* 如果使用的调制模式是LoRa模式 */
#if ((USE_MODEM_LORA == 1) && (USE_MODEM_FSK == 0))
  /* 打印LoRa调制信息，包括带宽和扩频因子 */
  APP_LOG(TS_OFF, VLEVEL_M, "---------------\n\r");
...
#error "Please define a modulation in the subghz_phy_app.h file."
  ...
  /*fills tx buffer 填充发送缓冲区，使用memset将发送缓冲区BufferTx的内容初始化为0 */
  memset(BufferTx, 0x0, MAX_APP_BUFFER_SIZE);
  /* 打印随机延迟时间 */
  APP_LOG(TS_ON, VLEVEL_L, "rand=%d\n\r", random_delay);
  /*starts reception 启动接收操作，设定超时时间+random_delay */
  Radio.Rx(RX_TIMEOUT_VALUE + random_delay);

  /*register task to to be run in while(1) after Radio IT 注册任务PingPong_Process，这个任务将在主循环while(1)中运行 */
