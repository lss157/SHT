...
void SystemApp_Init(void)
{
  ...
  /* Ensure that MSI is wake-up system clock 使用主系统时钟（MSI）作为唤醒时钟 */
  __HAL_RCC_WAKEUPSTOP_CLK_CONFIG(RCC_STOP_WAKEUPCLOCK_MSI);

  /*Initialize timer and RTC 初始化定时器和RTC SYS_TimerInitialisedFlag表示定时器已经被成功初始化 */
  UTIL_TIMER_Init();
  ...
  /*Init low power manager 初始化低功耗管理器 */
  UTIL_LPM_Init();
  /* Disable Stand-by mode 禁用standby模式 */
