 ...
 * @LastEditors: jp
 * @Description: 
 * @FilePath: \SHT-stm32f103-master\Core\Src\SHT\DHT11\src\dht11.c
 */
#include "dht11.h"
#include "sys_app.h"

extern TIM_HandleTypeDef htim16;
uint8_t TIM16_WaitSignal = 0;


void tim16_delay_us(uint16_t delay){
	if(TIM16_WaitSignal) {
		return;
	}
	TIM16_WaitSignal = 0;
	htim16.Instance -> CNT = 0x00;
    htim16.Init.Period = (uint32_t)delay;
	htim16.Instance -> ARR = htim16.Init.Period - 1;
	HAL_TIM_Base_Start_IT(&htim16);
	while(!TIM16_WaitSignal);
	HAL_TIM_Base_Stop_IT(&htim16);
	TIM16_WaitSignal = 0;
}

void TIM16_PeriodElapsedCallback() {
	if(!TIM16_WaitSignal) {
		TIM16_WaitSignal = 1;
	}
}
...
//    Buffer.temp_humi_period = (((period_private_data_t *)sht_cur_thread->private_data)->time)/1000;
//    long level = sht_enter_critical();
//    Buffer.temp_collect_time = timestap;
//    sht_exit_critical(level);
//    data_ready|=(1<<1);
      APP_LOG(TS_OFF, VLEVEL_M, "humi:%d.%d\ttemp:%d.%d\r\n",DHT_Buffer[0],DHT_Buffer[1],DHT_Buffer[2],DHT_Buffer[3]);
  }
  else
  {
    APP_LOG(TS_OFF, VLEVEL_M, "collect temp failed!!\r\n");
  }
}
